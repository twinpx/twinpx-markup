...
function Form(elem) {
    ...
    function handleEvents() {
        ...
        self.$elem.find("input, textarea").focus(focusElement).keyup(keyupElement);
        ...
        if (isValid()) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.url) {
                        self.$elem.find(".b-form-message__back").attr({
                            href: data.url
                        });
                    }
    ...
    function isValid() {
        ...
        function checkEqual() {
            ...
            self.$elem.find("[data-equal]").each(function() {
                var $filed = $(this), data = $filed.attr("data-equal");
        ...
        function checkEmpty() {
            ...
            self.$elem.find("[required]").each(function() {
                var $field = $(this), $val = $.trim($field.val());
                if ($field.is("[type=radio]")) {
                    if ($field.closest(".b-form-field").find("input:checked").size() == 0) setAttention($field);
        ...
        function checkSpecialTypes() {
            ...
            function checkPasswordType() {
                self.$elem.find("input:visible[type=password]").each(function() {
                    var $field = $(this), $val = $.trim($field.val()), num = 6;
                    if ($val.length < num) {
                        setAttention($field);
                    } else {
                        removeAttention($field);
            ...
            function checkEmailType() {
                self.$elem.find("[type=email]").each(function() {
                    var $field = $(this), $val = $.trim($field.val()), mailRegex = /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i;
            ...
            function checkTelType() {
                self.$elem.find("[type=tel]").each(function() {
                    var $field = $(this), $val = $.trim($field.val()), phoneRegex = /^([0-9-()\++\s]{5,})$/i;
            ...
            function checkNumberType() {
                self.$elem.find("[type=number]").each(function() {
                    var $field = $(this), $val = $.trim($field.val()), numRegex = /^([0-9\s\.,]+)$/i;
            ...
            function checkUrlType() {
                self.$elem.find("[type=url]").each(function() {
                    var $field = $(this), $val = $.trim($field.val()), urlRegex = /^((https?:\/\/)?(www\.)?([-a-z0-9]+\.)+[a-z]{2,})$/i;
        ...
        function checkRequiredOr() {
            ...
            self.$elem.find("[data-required-or]").each(function() {
                var $filed = $(this), data = $filed.attr("data-required-or");
                if (!orFieldsObject[data]) {
...
(function($) {
    ...
    $.fn.placeholder = function(params) {
        ...
        $(this).each(function() {
            var self = this;
            self.$elem = $(this), self.$formField = self.$elem.closest(".b-form-field");
            ...
            function handleEvents() {
                self.$placeholderText.click(function() {
                    if (!self.$elem.parent().hasClass("i-disabled")) self.$elem.focus();
                });
                self.$elem.focus(function() {
                    onFocus();
                }).blur(function() {
                    onBlur();
                });
...
function adaptiveBlocks() {
    ...
    function adaptive(elem) {
        ...
        $(window).bind("resize.adaptive", resize).resize();
        function getWidth() {
...
function ScrolledMenu($elem) {
    ...
    function handleEvents() {
        self.$elem.delegate(".b-scrolled-menu__nav-top", "mouseenter", enter).delegate(".b-scrolled-menu__nav-top", "mouseleave", leave).delegate(".b-scrolled-menu__nav-top", "click", false).delegate(".b-scrolled-menu__nav-bottom", "mouseenter", enter).delegate(".b-scrolled-menu__nav-bottom", "mouseleave", leave).delegate(".b-scrolled-menu__nav-bottom", "click", false);
        self.$wrapper.mousewheel(function(e) {
            if (!self.$elem.hasClass("i-top") && !self.$elem.hasClass("i-bottom")) return;
            e.preventDefault();
            self.intervalCounter = 100;
            if (e.deltaY > 0) increaseMarginTop(true); else if (e.deltaY < 0) increaseMarginTop(false);
        });
    }
    function enter() {
        if (isMobile()) return;
    ...
    function increaseMarginTop(increase) {
        if (self.intervalCounter < 3) self.intervalCounter += .2;
...
function Clients($elem) {
    ...
    function handleEvents() {
        self.$elem.delegate(".b-clients__item", "mouseenter", mouseenterItem).delegate(".b-clients__item", "mouseleave", mouseleaveItem);
    }
    function mouseenterItem() {
        $(this).find(".b-clients__item__bg").stop().animate({
            opacity: .87
        }, 500).end().find(".b-clients__item__name").stop().animate({
            top: "50%"
        }, 800, "easeOutElastic");
    }
...
function Sliding(elem) {
    ...
    function initVarsAndElems() {
        ...
        self.$slidingContent = self.$elem.find(".b-sliding-content").addClass("i-fadeIn");
    ...
    function slide(url) {
                ...
                function success(response, status, xhr) {
                    setTimeout(function() {
                        ...
                        if (self.$elem.hasClass("i-preloader")) {
                            ...
                            self.$slidingContent.addClass("i-fadeIn");
                            self.fadeFlag = true;
        ...
        function slideOut() {
            ...
            if (self.$slidingContent.length !== 0) {
                self.$slidingContent.removeClass("i-fadeIn");
                self.$elem.attr({
                    "data-marginLeft": self.$elem.css("marginLeft")
                });
                self.$elem.addClass("i-slideOut");
                self.timeoutId = setTimeout(slideIn, 500);
            } else {
                self.$elem.attr({
                    "data-marginLeft": self.$elem.css("marginLeft")
                });
...
function MainMenu() {
    ...
    function handleEvents() {
        self.$elem.delegate("a", "mouseenter", enterLink).delegate("a", "mouseleave", leaveLink);
    ...
    function highlight() {
        ...
        if (!url) return;
        self.$mainSubmenu.find("a").each(function() {
            var $a = $(this);
        ...
        self.$mainMenu.find("a").each(function() {
            var $a = $(this);
            var $subActive = self.$mainSubmenu.find(".i-active");
            if ($subActive.size() == 0) $subActive = self.$mainSubmenu.find(".i-inside");
            if ($subActive.size() == 0) {
...
function contactsNav() {
    ...
    $contacts.mouseenter(enter).mouseleave(leave);
    function enter() {
        if (isMobile()) return;
        animationFlag = true;
        setTimeout(function() {
...
function Portfolio($elem) {
    ...
    function handleEvents() {
        self.$elem.delegate(".b-portfolio__item__link", "mouseenter", mouseenterLink).delegate(".b-portfolio__item__link", "mouseleave", mouseleaveLink);
    }
    function mouseenterLink() {
        $(this).find(".b-portfolio__item__link__bg").stop().animate({
            opacity: .75
        }, 500).end().find(".b-portfolio__item__link__name").stop().animate({
            top: "25px"
        }, 800, "easeOutElastic");
    }
